...
var Carousel = _react2['default'].createClass({
  ...
  getDefaultProps: function getDefaultProps() {
    return {
      ...
      cellSpacing: 0,
      data: function data() {},
      decorators: _decorators.DefaultDecorators,
      dragging: true,
      easing: 'easeOutCirc',
  ...
  getInitialState: function getInitialState() {
    ...
    for (var i = 0, _length = _react2['default'].Children.count(this.props.children); i < _length; i++) {
  ...
  setLazyLoadList: function setLazyLoadList(props) {
    ...
    var min = Math.max(0, currentSlideIndex - lazyLoadingBuffer * slidesToShow);
    var max = Math.min(children.length, currentSlideIndex + slidesToShow + slidesToShow * lazyLoadingBuffer);

    var lazyLoadList = props.children.map(function (el, index) {
      return index >= min && index <= max || (self.state.lazyLoadList[index] === undefined ? false : self.state.lazyLoadList[index]);
    });

    this.setState({ lazyLoadList: lazyLoadList });
  },
  ...
  addToLazyLoadList: function addToLazyLoadList() {
    ...
    var min = Math.max(0, selectedSlideIndex - lazyLoadingBuffer * slidesToShow);
    var max = Math.min(children.length, selectedSlideIndex + slidesToShow + slidesToShow * lazyLoadingBuffer);

    var lazyLoadList = this.state.lazyLoadList.map(function (el, index) {
      return index >= min && index <= max || el;
    });
  ...
  formatChildren: function formatChildren(children) {
    ...
    return _react2['default'].Children.map(children, function (child, index) {
      ...
      return _react2['default'].createElement(
        ...
        self.props.lazyLoading ? lazyLoadList[index] && child : child
      );
    });
  },
  ...
  getSlideStyles: function getSlideStyles() {
    return {
      ...
      height: this.props.lazyLoading && this.props.vertical ? this.state.slideWidth : 'auto',
      boxSizing: 'border-box',
      MozBoxSizing: 'border-box',
